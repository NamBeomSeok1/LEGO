...
$(document).on("click", '#editBtn', function(){
	...
	if ($(".goodsLi").length == 0 && $(".option-select-area li[data-secode=D]").length == 0) {
		...
		if(fnGoodsCountChk()){
			if (goodsKndCode == 'GNR') {
				...
				$(".option-select-area li").each(function () {
					...
					if ($(this).data("secode") == "D") {
						...
						if(!!$(this).data("cartid")){//기존 옵션
							updateCartList.push({
								"cartNo": $(this).data("cartid"),
							...
							if ($(this).find(".liChdrId").length != 0) {
				...
				if(newCartList.length != 0){
					jsonData.cartVoList = newCartList;
				}

				if(updateCartList.length != 0){
					jsonData.cartItemList = updateCartList;
				}
			...
			$.ajax({
				url: CTX_ROOT + '/shop/goods/updateCart2.json',
...
<input type="hidden" id="cartGroupNo"  value="${cartGroupList[0].cartGroupNo }"/>
	<c:choose>
	...
	<div class="pop-body">
							...
							<li>
								...
								<div class="datepicker-area">
									...
									<c:choose>
										...
										<c:otherwise>
											<select name="sbscrptDlvyDay" id="sbscrptDlvyDay" class="sbscrptDlvyDay" title="정기결제일 선택" style="width: 100%">
		...
		<div class="bg-area">
			<ul class="option-select-area">
				<c:if test="${not empty cartGroupList[0].cartList}">
						<c:forEach items="${cartGroupList[0].cartList}" var="result" varStatus="status">

								<c:set var="tempText" value=""/>
								<c:set var="tempGitemSeCode" value=""/>
							...
							<c:forEach items="${result.cartItemList}" var="item" varStatus="itemStatus">
								<c:choose>
									<c:when test="${item.gitemSeCode eq 'Q'}">
										<c:set var="tempText" value="${tempText} / ${item.gitemAnswer} "/>
									</c:when>
									<c:when test="${item.gitemSeCode eq 'D'}">
										<c:set var="tempText" value="${tempText}/${fn:replace(item.gitemNm, ',' ,'/')}"/>
										<c:set var="tempGitemSeCode" value="${item.gitemSeCode}"/>
									...
									<c:when test="${item.gitemSeCode eq 'S'}">
							...
							<c:if test="${not empty result.chldrnId and tempGitemSeCode ne 'S'}">
							...
							<c:if test="${goods.optnUseAt eq 'N' and goods.goodsKndCode eq 'GNR'}">
								<c:set var="oneGoods" value="true"/>
							</c:if>

							<li class="${oneGoods ? 'goodsLi' : oneGoods}" data-secode="${tempGitemSeCode}" data-cartid="${result.cartNo}" data-optid="${tempGitemId}${tempChlId}${tempChlNm}">
