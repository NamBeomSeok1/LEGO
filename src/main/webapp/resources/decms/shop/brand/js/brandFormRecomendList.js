(function() {
		...
		columns: [
			...
			{ header: '선택', name: '', width: 100, align: 'center',
				formatter: function(item) {
					...
					+ 'data-goods-id="' + item.row.goodsId + '" data-goods-nm="' + item.row.goodsNm + '" data-prtnrid="' + item.row.prtnrId + '" data-evt-txt="' + item.row.goodsIntrcn + '"data-regist-sttus-code="'+ item.row.registSttusCode + '"><i class="fas fa-check"></i></button>';
				}
			},
		],
		columnOptions: {
	...
	$(document).on('click', '.btnAddRecomend', function(e) {
		...
		var prtnrId = $(this).data('prtnrid');
		var cmpnyId = $('#cmpnyId').val();
		
		if(isEmpty(cmpnyId)){
			bootbox.alert({ title:'확인',message: '업체 등록 후 이용가능합니다.', size: 'small' });
			return false;
		}
		$modal.modal('show');
		$modal.on('shown.bs.modal', function(e) {
			
			$('#recomendSearchForm').find('select[name=searchPrtnrId]').val(prtnrId);
			$('#recomendSearchForm').find('input[name=searchCmpnyId]').val(cmpnyId);
			grid.refreshLayout();
			$('#recomendSearchForm').submit();
		});
	});
	...
	$(document).on('click', '.btnSelectRecomend', function(e) {
		...
		var goodsId = $(this).data('goodsId'),
			...
			prtnrId = $(this).data('prtnrid'),
			evtTxt = $(this).data('evt-txt'),
			registCode = $(this).data('regist-sttus-code')

		if(registCode=='R'){
			bootbox.alert({ title:'확인',message: '등록대기 중인 상품입니다.', size: 'small' });
			return false;
		}
		if(prtnrId == 'PRTNR_0000'){
			$('#btcGoodsNm').val(goodsNm);
			$('#brandBtcGoodsId').val(goodsId);
		}else if(prtnrId == 'PRTNR_0001'){
			$('#btbGoodsNm').val(goodsNm);
			$('#brandBtbGoodsId').val(goodsId);
		}
	...
	$(document).on('click', '.btnDeleteRecomend', function(e) {
		...
		bootbox.confirm({
			...
			callback: function(result) {
				if(result) {
					deleteRecomend(actionUrl);
					$self.parents('.recomend-item').remove();
					reIndexRecomend();
				}
			}
		});
