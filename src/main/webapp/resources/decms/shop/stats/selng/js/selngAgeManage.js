(function() {
	...
	const grid = new tui.Grid({
		...
		columns: [
			{ header: '연령', name: 'agrde', width:120, align: 'center'},
	...
	function drawAgeOrderChart(chartTitle, dataList) {
		$('#chart1').html('');
	...
	//매출
	function drawAgeSelngChart(chartTitle, dataList) {
		$('#chart2').html('');
		if(dataList.length == 0) return;
		...
		$.ajax({
			...
			success: function(result) {
				...
				if(result.success) {
					...
					drawAgeOrderChart('연령별 주문건수',result.data.list);
					drawAgeSelngChart('연령별 매출',result.data.list);
				}
			}
		});
	};
