(function() {
	const grid = new tui.Grid({
		...
		columns: [
			{ header: '기간', name: 'mde', width:130, align: 'center'},
	...
	function drawMonthOrderChart(chartTitle, dataList) {
		$('#chart1').html('');
	...
	//주별 매출
	function drawMonthSelngChart(chartTitle, dataList) {
		$('#chart2').html('');
		if(dataList.length == 0) return;
		...
		$.ajax({
			...
			success: function(result) {
				...
				if(result.success) {
					...
					drawMonthOrderChart('월별 주문건수',result.data.list);
					drawMonthSelngChart('월별 매출', result.data.list);
				}
			}
		});
	};
