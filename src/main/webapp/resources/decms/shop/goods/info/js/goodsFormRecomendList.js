(function() {
	...
	$(document).on('click', '.btnAddRecomend', function(e) {
		...
		$modal.on('shown.bs.modal', function(e) {
			...
			if($('input[name=prtnrId]:checked').length > 0) {
				prtnrId = $('input[name=prtnrId]:checked').val();
				console.log(prtnrId);
	...
	$(document).on('click', '.btnSelectRecomend', function(e) {
		...
		if(type == "banner"){
			$("#goodsId").val(goodsId);
			$("#goodsNm").val(goodsNm);
			$("#bannerLink").val(CTX_ROOT + "/shop/goods/goodsView.do?goodsId=" + goodsId);
			$("#evtTxt").val(evtTxt);
			$('input[name=prtnrId]:input[value=' + prtnrId +']').attr('checked', true);
