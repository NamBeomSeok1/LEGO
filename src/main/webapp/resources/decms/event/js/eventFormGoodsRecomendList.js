(function() {
	...
	$(document).on('click', '.btnSelectRecomend', function(e) {
		...
		if(registCode=='R'){
			...
			addGoodsMapng(goodsId, goodsNm);
		}
		
	});
	
	addGoodsMapng = function(goodsId, goodsNm) {
		//console.log(parentGridGoods);
		var row = {
			'goodsId' : goodsId
			, 'goodsNm' : goodsNm
			/*, 'manage' : '<a href="#" class="btn btn-success" title="수정">' 
						+ 		'<i class="fas fa-edit"></i></a>' 
						+ 	'<a href="#"  class="btn btn-danger" title="삭제">' 
						+ 		'<i class="fas fa-trash"></i></a>'*/
		
		};
		var options = {
			'at' : parentGridGoods.getRowCount()
			, 'focus' : false
		};

		parentGridGoods.appendRow(row, options);
