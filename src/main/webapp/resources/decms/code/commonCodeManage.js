(function() {
	const grid = new tui.Grid({
		...
		columns: [
			{ header: '코드ID', name:'codeId', width: 90, align: 'center'},
			{ header: '코드명', name: 'codeIdNm', width: 100, align: 'center' },
			...
			{ header: '관리', name: '', width: 100, align: 'center',
				formatter: function(item) {
					return 	'<a href="' + CTX_ROOT + '/decms/embed/code/modifyCode.do?codeId=' + item.row.codeId + '" class="btn btn-success btn-sm mr-1 btnModify" data-target="#codeModal" title="수정">' 
						+ 		'<i class="fas fa-edit"></i></a>' 
						+ 	'<a href="' + CTX_ROOT + '/decms/code/deleteCode.json?codeId=' + item.row.codeId + '" class="btn btn-danger btn-sm btnDelete" title="삭제">' 
	...
	grid.on('click', function(e) {
		...
		dataList.forEach(function(item) {
			grid.removeRowClassName(item.rowKey, 'bg-warning');
		});

		grid.addRowClassName(e.rowKey, 'bg-warning');
		
		var item = grid.getRow(e.rowKey);
		$('#searchSubForm').find('#codeId').val(item.codeId);
	...
	$(document).on('click', '.btnAdd', function(e) {
		...
		modalShow(actionUrl, '코드 등록', modal);
	});
	...
	$(document).on('click', '.btnModify', function(e) {
		...
		modalShow(actionUrl,'코드 수정', modal);
	});
