...
public class IniFunc {
	...
	Logger LOGGER = LoggerFactory.getLogger(IniFunc.class);
	...
	public static void arraycopy_system(byte[] src, int src_offset, byte[] dst, int dst_offset, int length) {
		for (int i = 0; i < length; i++) {
			dst[dst_offset + i] = src[src_offset + i];
		}
	}
	...
	public static int[] chartoint32_for_SEED_CBC(byte[] in, int inLen) {
		...
		for (i = 0; i < len; i++) {
			byte_to_int(data, i, in, i * 4, ENDIAN);
		}
	...
	public static int SEED_CBC_init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, byte[] pbszUserKey, byte[] pbszIV) {
		...
		memcpy(pInfo.ivec, pbszIV, 16, ENDIAN);
		pInfo.last_block_flag = pInfo.buffer_length = 0;

		// Set up input values for Key Schedule
		ABCD[ABCD_A] = byte_to_int(pbszUserKey, 0 * 4, ENDIAN);
		ABCD[ABCD_B] = byte_to_int(pbszUserKey, 1 * 4, ENDIAN);
		ABCD[ABCD_C] = byte_to_int(pbszUserKey, 2 * 4, ENDIAN);
		ABCD[ABCD_D] = byte_to_int(pbszUserKey, 3 * 4, ENDIAN);
	...
	public static int SEED_CBC_Process(KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			memcpy(pInfo.cbc_buffer, in, in_offset, pInfo.buffer_length);
		} else {
			pdwXOR = pInfo.ivec;
			in_offset = 0;
			out_offset = 0;
			...
			memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			memcpy(pInfo.cbc_last_block, out, out_offset - 4, BLOCK_SIZE_SEED);
		}
	...
	public static int SEED_CBC_Close(KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			for (i = pInfo.buffer_length; i < BLOCK_SIZE_SEED; i++) {
				set_byte_for_int(pInfo.cbc_buffer, i, (byte) nPaddngLeng, ENDIAN);
			}

			BLOCK_XOR_CBC(pInfo.cbc_buffer, 0, pInfo.cbc_buffer, 0, pInfo.ivec, 0);

			KISA_SEED_Encrypt_Block_forCBC(pInfo.cbc_buffer, 0, out, (out_offset) / 4, pInfo.seed_key);

			outLen[0] = BLOCK_SIZE_SEED;
		} else {
			nPaddngLeng = get_byte_for_int(pInfo.cbc_last_block, BLOCK_SIZE_SEED - 1, ENDIAN);
			if (nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED) {
				for (i = nPaddngLeng; i > 0; i--) {
					set_byte_for_int(out, out_offset - i, (byte) 0x00, ENDIAN);
				}
	...
	public static byte[] SEED_CBC_Encrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset,
		...
		byte[] pbszPlainText = new byte[nPlainTextLen];
		arraycopy_system(message, message_offset, pbszPlainText, 0, nPlainTextLen);
		...
		SEED_CBC_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng);

		cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0] + nPaddingLeng[0]);
		arraycopy(pbszCipherText, cdata, nRetOutLeng[0] + nPaddingLeng[0]);
	...
	public static byte[] SEED_CBC_Decrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset,
		...
		byte[] pbszCipherText = new byte[nCipherTextLen];
		arraycopy_system(message, message_offset, pbszCipherText, 0, nCipherTextLen);
		...
		arraycopy(newpbszCipherText, pbszCipherText, nCipherTextLen);
		...
		if (SEED_CBC_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng) > 0) {
			cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0] - nPaddingLeng[0]);
			arraycopy(pbszPlainText, cdata, nRetOutLeng[0] - nPaddingLeng[0]);
	...
	public String BinToHex(byte[] buf) {
		String res = "";
		String token = "";
	...
	public String seedDecrypt(String encryptTxt, String seedKey, String seedIv){
		
		byte[] cipherText; // 암호화 데이터
		byte[] key; // SEED KEY
		byte[] iv; // SEED IV
		
		String decryptTxt = "";
		
		cipherText = getBytes(BinToHex(Base64.getDecoder().decode(encryptTxt.getBytes())));
		key = getBytes(BinToHex(Base64.getDecoder().decode(seedKey.getBytes()))); // SEEDKEY BASE64DECODE
...
//		key = getBytes(BinToHex(DatatypeConverter.parseBase64Binary(seedKey))); // SEEDKEY BASE64DECODE
		...
		decryptTxt = getString(SEED_CBC_Decrypt(key, iv, cipherText, 0, cipherText.length));
		
			decryptTxt = new String(hexToByteArray(decryptTxt.replaceAll(",", "")), "euc-kr");
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public static String encrypteSHA256(String hashParam) throws Exception {
